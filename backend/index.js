...
const ADMIN_PASSWORD = '0000'; // 실제 운영 시에는 .env 파일로 관리하는 것이 좋습니다.
...
// 서버 시작 시 DB 테이블이 없으면 자동으로 생성하는 함수
...
app.get('/api/notices', async (req, res) => {
    try {
        ...
        // 참고: 페이지네이션 로직은 프론트엔드에서 더 쉽게 처리하거나,
        // SQL의 LIMIT, OFFSET을 사용하여 구현할 수 있습니다. 여기서는 전체 목록을 보냅니다.
        res.json({
...
app.delete('/api/bookings/cancel/:id', async (req, res) => {
    ...
    try {
        ...
        if (result.rowCount === 0) return res.status(404).json({ message: '취소할 예약 정보를 찾을 수 없습니다.' });
...
app.get('/api/bookings/status', async (req, res) => {
    ...
    try {
        const result = await pool.query(
            "SELECT deck_name FROM bookings WHERE booking_date = $1 AND section = $2 AND status != '반납 완료'",
            [date, section]
        );
...
app.listen(PORT, () => {
    ...
    // 서버가 시작될 때 데이터베이스 테이블 설정을 실행합니다.
    setupDatabase();
});
